...
class Application_Model_Testvalidateaadhar extends Zend_Db_Table_Abstract {
...
public function aadharvalidatebyuidai($scheme_id = null){
		...
		$select = $select_table->select();
		$select->setIntegrityCheck(false);
		$select->from(array('tbname' => $tabname),array('id','uniq_user_id','name','dob','email_id','state_name','aadhar_seeded','aadhar_num','tbname.amount','tbname.transaction_date','tbname.fund_transfer','tbname.state_code','tbname.state_name','tbname.gender','tbname.pincode','tbname.aadhar_validate','tbname.uidai_aadhar_validate','tbname.mobile_num','tbname.created'));
		$select->join(array('scheme'=>'dbt_scheme'),"tbname.scheme_id=scheme.id",array('scheme_type','id as scheme_id','ministry_id as min_id'));
		$select->where('tbname.scheme_id = ?', $scheme_id);
		$select->where('scheme.id = ?', $scheme_id);
		$select->where('tbname.aadhar_num != ""');
...
// for validate aadhaar number by uidai model start now --------------
	...
	public function nonuidaiaadharvalidate($scheme_id = null){
		...
		$select = $select_table->select();
		$select->setIntegrityCheck(false);
		$select->from(array('tbname' => $tabname),array('id','uniq_user_id','name','dob','email_id','state_name','aadhar_seeded','aadhar_num','tbname.amount','tbname.transaction_date','tbname.fund_transfer','tbname.state_code','tbname.uidai_aadhar_validate','tbname.mobile_num','tbname.created'));
		$select->join(array('scheme'=>'dbt_scheme'),"tbname.scheme_id=scheme.id",array('scheme_type','id as scheme_id','ministry_id as min_id'));
		$select->where('tbname.scheme_id = ?', $scheme_id);
		$select->where('scheme.id = ?', $scheme_id);
		$select->where('tbname.aadhar_num != ""');
