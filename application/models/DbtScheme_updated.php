...
class Application_Model_DbtScheme extends Zend_Db_Table_Abstract 
{
	// protected $_name = 'roles';
		...
		public function createTable($schemename = null, $schemeid = null){
           ...
           // echo "Aaaa";exit;
           // echo substr(preg_replace('/[^A-Za-z0-9]/', '', strtolower($schemename)), 0, 40);exit;
            $tablename = substr(preg_replace('/[^A-Za-z0-9]/', '', strtolower($schemename)), 0, 40)."_".$schemeid;
            //echo $tablename;exit;
            $update = new Zend_Db_Table("dbt_scheme");
    		$updateval = '';
    		$updateval = array('scheme_table'=> $tablename);
    		$where = array('id = ?'=> $schemeid);
    		$update->update($updateval,$where);
    		$tb_name = $tablename."_".$start."_".$dateended;
