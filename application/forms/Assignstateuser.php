<?PHP
class Application_Form_Assignstateuser extends Zend_Form{
	...
	$role = new Application_Model_Assignstateuser; 
 ...
 /*****************************************************************************/    
    public function editForm(){
        ...
        $required->setType ($required->getType() | Zend_Validate_NotEmpty::INTEGER | Zend_Validate_NotEmpty::ZERO);
	$customer_collect->addValidators (array ($required));
        $showrolename  = $role->userListEdit(base64_decode($_GET['userid']));
		
        foreach($showrolename as $key => $value){
            $name = $value['firstname'].' '.$value['lastname'];			
            $customer_collect->addMultiOption($value['id'], $name);
        }
        $pmArray=$role->assignedmanager(base64_decode($_GET['userid']));
//die("djked");
	...
	$role_collect = new Zend_Form_Element_MultiCheckbox('projectname',array(	    'label' => '',
        ...
        $role_collect->addValidators (array ($required));
        //echo $_GET['id'];exit;
        $pmArray11=$role->selectpidList(base64_decode($_GET['userid']));
        $showprojectname=$pmArray11->toArray();
        //echo "<pre>";
       // print_r($showprojectname);exit;
        ...
        $pmArray1=$role->showAssignStateUser(base64_decode($_GET['userid']));
