...
class Application_Form_Diststatesearchreport extends Zend_Form
{
...
//showing the state select option
        ...
        //echo $roleid->role;exit;
            ...
            if($roleid->role == 2 || $roleid->role == 5){
                $opted = $ministryget->selectUserstate($userid->userid);
                //echo "<pre>";print_r($opted);exit;
                foreach($opted as $list) {
                    $idstate = $list->state;
                }
                $state->setValue($idstate);
                 //$state->setValue($idstate);
            }
...
//showing the district select option
        ...
        if($roleid->role == 5){
            $opted = $ministryget->selectUserstate($userid->userid);
            foreach($opted as $list) {
                $cityid = $list->cityname;
                $district->setValue($cityid);
            }
        }
...
//showing the block select option
    ...
    if($idstate != "" && $cityid != ""){
        ...
        $blockData = $ministryget->selectblock($cityid);
            //echo "<pre>";
            //print_r($blockData->toArray());exit;
            foreach($blockData as $list) {
                //echo $blockname->blockcode."_".$list->blockname."<br />";
                $blockname = ucfirst($list->blockname);
                $block->addMultiOption($list->blockcode, $blockname);
            }
         }
