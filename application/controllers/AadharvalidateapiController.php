...
Class AadharvalidateapiController extends Zend_Controller_Action{
...
/*  --------------- aadhar validate by ncpi controller start now-------------*/ 
	...
	public function validateaadharAction(){
		...
		$status = safexss($request->getParam('status'));
...
//die("jsdfh");
		...
		for($i=0;$i<sizeof($show_list);$i++){
			...
			$txn = "NIC".$aadhaar_no.date("YmdHis").TXN;
			...
			<Pa ms=\"MS\"  state=\"$state_name\"></Pa><Pfa ms='".MS_AD."' mv='".MV."' av=\"\" lav=\"\" lmv=\"\"/>
			</Demo></Pid>";
			...
			//DECRYPT_FILE
			// generate auth xml
			$auth_xml = '<Auth ac="'.AC.'" lk="'.LK.'" sa="'.SA.'" tid="'.TID.'" txn="'.$txn.'" uid="'.$aadhaar_no.'" ver="'.API_VERSION.'"><Uses bio="n" otp="n" pa="y" pfa="n" pi="y" pin="n"/><Meta fdc="NA" idc="NA" lot="P" lov="'.$pincode.'" pip="'.PIP.'" udc="'.UDC.'"/><Skey ci="'.$this->public_key_validity().'">'.$this->encrypt_session_key($session_key).'</Skey><Data type="X">'.$this->encrypt_pid($pid_block, $session_key).'</Data><Hmac>'.$this->calculate_hmac($pid_block, $session_key).'</Hmac></Auth>';
//echo $auth_xml;exit;
...
/*  --------------- non aadhar uidai validate controller start now-------------*/ 
	...
	public function nonaadharuidaivalidateAction(){
		...
		if(isset($dataform['validate']) && $dataform['validate']=='Aadhaar  validation by UIDAI – Demo Authentication'){
		$aadhar_status_true=0;
		$aadhar_status_false=0;
		for($i=0;$i<sizeof($show_list);$i++){
		...
		//print_r($show_list);die;
			...
			$pincode=$show_list[$i]['pincode'];
			$pid_block="<Pid ts=\"$ts\" ver=\"1.0\"><Demo>
			<Pi ms='".MS."' name=\"$name\" gender=\"$gender\" dobt=\"D\"></Pi>
			<Pa ms='".MS."' state=\"$state_name\"></Pa><Pfa ms='".MS_AD."' mv='".MV."' av=\"\" lav=\"\" lmv=\"\"/></Demo></Pid>";
			//echo $pid_block;exit;
			//echo $pid_block;exit;
			// generate aes-256 session key
			$session_key = openssl_random_pseudo_bytes(32);
...
/*  --------------- non aadhar seeded validate controller start now-------------*/ 
	...
	public function nonaadharseededvalidateAction(){
		...
		if(isset($dataform['validate']) && $dataform['validate']=='NPCI validation for Aadhaar linked Bank Account'){
		$aadhar_status_true=0;
		$aadhar_status_false=0;
		for($i=0;$i<sizeof($show_list);$i++){
...
public function deletebeneficiaryAction()
	...
	if($admname->adminname==''){
           ...
           $scheme_id = safexss(base64_decode($request->getParam('scheme_id')));
		   $beneficiary = safexss(base64_decode($request->getPost('beneficiary')));
...
// echo "<pre>";print_r($schemedetails);echo "<pre>";print_r($getbeneficiarydetail);echo "<pre>";die;
		...
		if($getbeneficiarydetail>0 && $schemedetail>0 && $scheme_code>0){
			...
			// print_r($getbeneficiarydetail); exit;
			$schemetablename = 'dbt_'.$scheme_details_data['0']['scheme_table'];
			$beneficiariesids = array();
					...
					<CstmrInf>
						...
						<CstmrInfDt>'.$CstmrInfDt.'</CstmrInfDt>';
							...
							for($i=0;$i<sizeof($getbeneficiarydetail);$i++){
								$beneficiariesids[$i] = $getbeneficiarydetail[$i]['uniq_user_id'];
								$xmlfile .= '
							<CstmrTxInf>
								<OrgId>
									<Cd>'.$OrgId_Cd.'</Cd>
								...
								<Cstmr>
									...
									<Tp>'.$getbeneficiarydetail[$i]['beneficiary_type'].'</Tp>
									...
									<PstlAdr>
										...
										<Prtry>
											...
											<PrvcCd>'.$getbeneficiarydetail[$i]['state_code'].'</PrvcCd>
											<CtryCd>91</CtryCd>
										</Prtry>
			...
			fclose($myfile);
			// print_r($beneficiariesids); exit;
			$updatexmlstatus = $cmi_list->updatexmlstatus($schemetablename,$beneficiariesids);
			$updatebenreqcount = $cmi_list->updatebenreqcount(date('d-m-Y'),$sno);
			// print_r($updatebenreqcount); exit;
			
			$xmlfile = ''; //Unset xmlfile value;
			
			if($updatebenreqcount){
	...
	}//createpfmsxmlbeneficiaryAction
	...
	//Get xml beneficiary response
	public function xmlbeneficiaryresponseAction(){
		...
		$valid_aadhar = new Application_Model_Validateaadhar();// call model here
		$filename = "pfmsresponcetodbt.xml";
		// $myfile = fopen("/var/www/html/pfms/benrequest/".$filename, "r") or die("Unable to open file!");
		
		$xml=simplexml_load_file("/var/www/html/pfms/benrequest/".$filename);
		$json = json_encode($xml);
		...
		$noofrecords = $responsedata['CstmrDtls']['GrpHdr']['NbOfTxs']; //No of records
		$MsgId = $responsedata['CstmrDtls']['GrpHdr']['MsgId']; //Unique Message Id
		$CreDtTm = $responsedata['CstmrDtls']['GrpHdr']['CreDtTm']; //Message Date Time
		$GrpStsRsnInfCd = $responsedata['CstmrDtls']['GrpHdr']['OrglInitgPty']['GrpStsRsnInf']['Rsn']['Cd']; //Group Rejection Reason Code
		$GrpStsRsnInfAddtlInf = $responsedata['CstmrDtls']['GrpHdr']['OrglInitgPty']['GrpStsRsnInf']['Rsn']['AddtlInf']; //Group Rejection Reason Narration
		//echo $GrpStsRsnInfCd;die("test");
		
		if(empty($GrpStsRsnInfCd)){
		if ($noofrecords > 1) {
			$suss=0;
			$fail=0;
			for($i=0; $i<$noofrecords;$i++) {
				$SchmeCd = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['OrgId']['SchmeCd']; //Scheme Code
				$CPSMSId = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['CPSMSId']; //PFMS Beneficiary Code
				$Tp = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['Tp']; //Beneficiary type
				$CstmrSts = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['CstmrSts']; //Beneficiary Status
				$StsRsnInfCd = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['StsRsnInf']['Rsn']['Cd']; //Rejection Reason Code
				$StsRsnInfAddtlInf = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['StsRsnInf']['Rsn']['AddtlInf']; //Rejection Reason Narration
				
				$bankname = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['CstmrAcct']['CstmrAgt']['FinInstnId']['BICFI']; //bank name

			$bankbranchcode = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['CstmrAcct']['CstmrAgt']['FinInstnId']['BrnchId']; //bank branch code

			$bankaccountnumber = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['CstmrAcct']['AcctId']['Othr']['BBAN']; //bank account number

			$beneficiaryname = $responsedata['CstmrDtls']['CstmrInf'][$i]['CstmrTxInf']['Cstmr']['CstmrAcct']['CstmrAcctFmly']['Nm']; //beneficiary name
			
				//echo $beneficiaryname;die;
				
				if($CstmrSts=='ACCP'){
					$updatedata=$valid_aadhar->updatebeneficiaryfrompfms($SchmeCd,$CPSMSId,$bankbranchcode,$bankname,$bankaccountnumber,$CstmrSts,$StsRsnInfCd,$StsRsnInfAddtlInf,$beneficiaryname);
					$suss+=1;
				}else{
					$updatedata=$valid_aadhar->updatebeneficiaryfrompfms($SchmeCd,$CPSMSId,$bankbranchcode,$bankname,$bankaccountnumber,$CstmrSts,$StsRsnInfCd,$StsRsnInfAddtlInf,$beneficiaryname);
					$fail+=1;
			...
			echo 'Successfully Updated total no. of beneficiary is '.$suss.'<br><br>';
			echo 'Fail updated total no. of beneficiary is '.$fail;
			
		} else if ($noofrecords == 1) {
			$SchmeCd = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['OrgId']['SchmeCd']; //Scheme Code
			$CPSMSId = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['CPSMSId']; //PFMS Beneficiary Code
			$Tp = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['Tp']; //Beneficiary type
			$CstmrSts = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['CstmrSts']; //Beneficiary Status
			$StsRsnInfCd = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['StsRsnInf']['Rsn']['Cd']; //Rejection Reason Code
			$StsRsnInfAddtlInf = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['StsRsnInf']['Rsn']['AddtlInf']; //Rejection Reason Narration

			$bankname = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['CstmrAcct']['CstmrAgt']['FinInstnId']['BICFI']; //bank name

			$bankbranchcode = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['CstmrAcct']['CstmrAgt']['FinInstnId']['BrnchId']; //bank branch code

			$bankaccountnumber = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['CstmrAcct']['AcctId']['Othr']['BBAN']; //bank account number

			$beneficiaryname = $responsedata['CstmrDtls']['CstmrInf']['CstmrTxInf']['Cstmr']['CstmrAcct']['CstmrAcctFmly']['Nm']; //beneficiary name
			
			//echo $SchmeCd.'<br>'.$StsRsnInfCd.'<br>'.$StsRsnInfAddtlInf.'<br>'.$bankbranchcode.'<br>'.$bankname.'<br>'.$bankaccountnumber.'<br>'.$beneficiaryname;die;
			...
			if($CstmrSts=='ACCP'){
					$updatedata=$valid_aadhar->updatebeneficiaryfrompfms($SchmeCd,$CPSMSId,$bankbranchcode,$bankname,$bankaccountnumber,$CstmrSts,$StsRsnInfCd,$StsRsnInfAddtlInf,$beneficiaryname);
					$stattus+=1;
					echo 'Successfully Updated total no. of beneficiary is '.$stattus;
					}else{
					$updatedata=$valid_aadhar->updatebeneficiaryfrompfms($SchmeCd,$CPSMSId,$bankbranchcode,$bankname,$bankaccountnumber,$CstmrSts,$StsRsnInfCd,$StsRsnInfAddtlInf,$beneficiaryname);
					$stattus+=1;
					echo 'Fail Updated total no. of beneficiary is '.$stattus;
					}
			// }else{
				// $updatedata=$valid_aadhar->updatebeneficiaryfrompfms($CPSMSId,$bankbranchcode,$bankname,$bankaccountnumber,$CstmrSts,$StsRsnInfCd,$StsRsnInfAddtlInf); 
				// }
			//update query here
			} else{
				  }
			...
			echo "Sorry invalid xml format file!!!";
		}
		exit;
	
	}//xmlbeneficiaryresponseAction
	
	//Get xml payment response
	public function xmlpaymentresponseAction(){
		die("hello");
		$admname = new Zend_Session_Namespace('adminMname'); 
		$userid = new Zend_Session_Namespace('userid');
		...
		$filename = "pfmspayresponcetodbt.xml";
		// $myfile = fopen("/var/www/html/pfms/payrequest/".$filename, "r") or die("Unable to open file!");
		
		$xml=simplexml_load_file("/var/www/html/pfms/payrequest/".$filename);
		$json = json_encode($xml);
		$responsedata = json_decode($json, true);
		// print '<pre>';
		// print_r($responsedata);
		...
		$noofrecords = $responsedata['CstmrPmtAckRpt']['GrpHdr']['NbOfTxs']; //No of records
		$MsgId = $responsedata['CstmrPmtAckRpt']['GrpHdr']['MsgId']; //Unique Message Id
		$CreDtTm = $responsedata['CstmrPmtAckRpt']['GrpHdr']['CreDtTm']; //Message Date Time
		
		$OrgnlGrpInfAndStsMsgId = $responsedata['CstmrPmtAckRpt']['OrgnlGrpInfAndSts']['OrgnlMsgId']; //Request Message Id
		$OrgnlGrpInfAndStsCreDtTm = $responsedata['CstmrPmtAckRpt']['OrgnlGrpInfAndSts']['OrgnlCreDtTm']; //Request Date Time
		
		$GrpStsRsnInfSts = $responsedata['CstmrPmtAckRpt']['OrgnlGrpInfAndSts']['GrpSts']; //Group RJCT/ACPT
		$GrpStsRsnInfCd = $responsedata['CstmrPmtAckRpt']['OrgnlGrpInfAndSts']['GrpStsRsnInf']['Rsn']['Cd']; //Group Rejection Reason Code
		$GrpStsRsnInfAddtlInf = $responsedata['CstmrPmtAckRpt']['OrgnlGrpInfAndSts']['GrpStsRsnInf']['Rsn']['AddtlInf']; //Group Rejection Reason Narration

		if ($noofrecords > 1) {
			for($i=0; $i<$noofrecords;$i++) {
				$OrgnlEndToEndId = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts'][$i]['OrgnlEndToEndId']; //Orgnl End To End Id
				$DBTBeneficiaryId = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts'][$i]['PrTry']['Id']; //DBT Beneficiary Id
				$PFMSBeneficiaryId = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts'][$i]['CPSMSId']; //PFMS Beneficiary Id
				$PaymentSts = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts'][$i]['TxSts']; //Payment Status
				$PaymentRjctCd = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts'][$i]['StsRsnInf']['Rsn']['Cd']; //Payment Rejection Code
				$PaymentRjctMsg = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts'][$i]['StsRsnInf']['Rsn']['AddtlInf']; //Payment Rejection Message

				echo $PaymentSts.'<br>';
				//update query here
			}
		} else if ($noofrecords == 1) {
			$OrgnlEndToEndId = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts']['OrgnlEndToEndId']; //Orgnl End To End Id
			$DBTBeneficiaryId = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts']['PrTry']['Id']; //DBT Beneficiary Id
			$PFMSBeneficiaryId = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts']['CPSMSId']; //PFMS Beneficiary Id
			$PaymentSts = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts']['TxSts']; //Payment Status
			$PaymentRjctCd = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts']['StsRsnInf']['Rsn']['Cd']; //Payment Rejection Code
			$PaymentRjctMsg = $responsedata['CstmrPmtAckRpt']['OrgnlPmtInfAndSts']['TxInfAndSts']['StsRsnInf']['Rsn']['AddtlInf']; //Payment Rejection Message
			
			echo $PaymentRjctCd;
			//update query here
		} else {
			echo $GrpStsRsnInfSts;
			echo '<br>';
			echo $GrpStsRsnInfAddtlInf;
			echo '<br>';
			//update query here
		}
		exit;
	
	}//xmlpaymentresponseAction

// uidai decryption encryption function start now
